...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, ShopcatAdapter.CheckInterface, ShopcatAdapter.ModifyCountInterface, ShopcatAdapter.GroupEditorListener {
    ...
    private TextView mPayGo;
    private LinearLayout mInfoOrder;
    private TextView mGoodsShare;
    private TextView mGoodsCollect;
    private TextView mGoodsDel;
    private LinearLayout mInfoShare;
    private LinearLayout mCartLl;
    private PtrFrameLayout mMPtrframe;
    ...
    private void initView() {
        ...
        mCheckBoxAll = (CheckBox) findViewById(R.id.all_checkBox);
        mCheckBoxAll.setOnClickListener(this);
        mPriceTotal = (TextView) findViewById(R.id.total_price);
        mPayGo = (TextView) findViewById(R.id.go_pay);
        mInfoOrder = (LinearLayout) findViewById(R.id.order_info);
        mGoodsShare = (TextView) findViewById(R.id.share_goods);
        mGoodsCollect = (TextView) findViewById(R.id.collect_goods);
        mGoodsDel = (TextView) findViewById(R.id.del_goods);
        mInfoShare = (LinearLayout) findViewById(R.id.share_info);
        mCartLl = (LinearLayout) findViewById(R.id.ll_cart);
        mMPtrframe = (PtrFrameLayout) findViewById(R.id.mPtrframe);
    }
...
//        header.initWithString("xiaoma is good");
        ...
        header.setPadding(dp2px(20), dp2px(20), 0, 0);
        mMPtrframe.setHeaderView(header);
        mMPtrframe.addPtrUIHandler(header);
        mMPtrframe.setPtrHandler(new PtrDefaultHandler() {
            @Override
            public void onRefreshBegin(PtrFrameLayout frame) {
                mMPtrframe.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        mMPtrframe.refreshComplete();
                    }
                }, 2000);
            }
    ...
    private void initEvents() {
        ...
        mListView.setGroupIndicator(null); //设置属性 GroupIndicator 去掉向下箭头
        mListView.setAdapter(adapter);
        for (int i = 0; i < adapter.getGroupCount(); i++) {
            mListView.expandGroup(i); //关键步骤4:初始化，将ExpandableListView以展开的方式显示
        }
        mListView.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                ...
                if (firstVisibleItem == 0 && top == 0) {
                    mMPtrframe.setEnabled(true);
                } else {
                    mMPtrframe.setEnabled(false);
                }
            }
        });
    }
    ...
    private void setCartNum() {
        ...
        for (int i = 0; i < groups.size(); i++) {
            ...
            group.setChoosed(mCheckBoxAll.isChecked());
    ...
    public void checkGroup(int groupPosition, boolean isChecked) {
        ...
        if (isCheckAll()) {
            mCheckBoxAll.setChecked(true);//全选
        } else {
            mCheckBoxAll.setChecked(false);//反选
        }
        adapter.notifyDataSetChanged();
        calulate();
    }
    ...
    private void doCheckAll() {
        for (int i = 0; i < groups.size(); i++) {
            ...
            List<GoodsInfo> child = childs.get(group.getId());
            for (int j = 0; j < child.size(); j++) {
                child.get(j).setChoosed(mCheckBoxAll.isChecked());//这里出现过错误
            }
        }
        adapter.notifyDataSetChanged();
        calulate();
    ...
    private void calulate() {
        ...
        mPriceTotal.setText("￥" + mtotalPrice + "");
        mPayGo.setText("去支付(" + mtotalCount + ")");
        if (mtotalCount == 0) {
            setCartNum();
        } else {
    ...
    private void setVisiable() {
        if (flag) {
            mInfoOrder.setVisibility(View.GONE);
            mInfoShare.setVisibility(View.VISIBLE);
            actionBarEdit.setText("完成");
        } else {
            mInfoOrder.setVisibility(View.VISIBLE);
            mInfoShare.setVisibility(View.GONE);
