...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private String[] mVals = new String[]{"Java", "Android", "iOS", "Python",
            "Mac OS", "PHP", "JavaScript", "Objective-C","y","有",
            "Groovy", "Pascal", "Ruby", "Go", "Swift就是了等级分类登记法律上来荆防颗粒聚少离多开房记录数据分类考试东方闪电"};//数据模拟，实际应从网络获取此数据
    ...
    private List<String> mData = new ArrayList<>();//记录文本
    ...
    private void initData() {
        ...
        for (int i = 0; i < 1; i++) {
            mData.add("第"+new Random().nextInt(100)+1);
            mData.add("第一个条目"+i);
            mData.add("第一个条目将数据放入流式布局"+i);
            mData.add("第一个条目将数据放入流式布局条目将数据放入流式布局条目将数据放入流式布局条目将数据放入流式布局条目将数据放入流式布局"+i);
        }
        mFlowLayout.setData(mData);
...
//        });
    ...
    public void save() {
        ...
        System.out.println("当前需要保持的Text：+——"+text);
        System.out.println("oldText是否包含当前Text：+——"+oldText.contains(text));
